...
[user-groups](/admins/user_management/users_and_groups) within Onyx.
...
<Accordion title="Onyx system default Instruction">
  ...
  Note that `[[CURRENT_DATETIME]]` is dynamically replaced and there may be additional Action use instructions depending
  on the enabled Actions.
</Accordion>
...
<Note>
  If any of the built-in actions (Web Search, Internal Search, Image Generation, Code Execution) are missing,
