...
Currently, Onyx supports three models for image generation, all through OpenAI. In the future,
there likely will be support for other options such as Google Imagen or Stability AI.

**GPT-Image-1.5**: Newest model, best for production-quality generation, with strong prompt control and reliable in-image text/structured layouts
