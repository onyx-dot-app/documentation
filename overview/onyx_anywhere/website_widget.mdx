...
1. **Widget loads** - Lightweight JavaScript bundle initializes the web component
2. **Session created** - Widget creates a chat session via the Onyx API
3. **User sends message** - Request is sent to your Onyx backend
4. **Streaming response** - AI response streams back in real-time via SSE
5. **Markdown rendered** - Responses are formatted with full markdown support
...
For Onyx Cloud users, the widget is served from Onyx's CDN.
Simply add the script tag and configure your widget attributes.

### Self-Hosted Deployment

For self-hosted Onyx instances:

1. Build the widget from the `widget/` directory in the Onyx repository
2. Deploy `dist/onyx-widget.js` to your own CDN or server
