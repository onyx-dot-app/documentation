...
## Indexing Flow
Documents from connectors and user uploads are processed by the Onyx indexing pipeline. With default configurations no data ever leaves the deployment.

The general processing outline goes as follows:
1. Documents, metadata, and access permissions are pulled in from connectors
2. Documents are processed into text through document parsing utilities
3. The texts are chunked and passed through deep learning (embedding) models
4. These representations are stored in the vector database
5. Optionally (default off), an LLM can be used to extract entities and relations from the documents and represent them as a graph within Postgres
...
If additional context is needed, the system can choose between available options including: ingested knowledge, internet search (if configured), build in actions (like code interpreter), or additional user configured actions.
...
**Web Search**: Sends search queries to a configured search provider. Supported providers include Google PSE, Serper, and Exa AI to get links and snippets. A crawler is used to fetch the full contents of the page, Onyx has a built in one and also supports Firecrawl.
