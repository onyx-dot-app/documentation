...
Onyx leverages Large Language Models (LLMs) to power its conversational AI capabilities, enabling intelligent responses, document analysis, and automated workflows. This guide explains how Onyx uses LLMs and how administrators can configure multiple providers to offer users flexibility in choosing models for different use cases.
...
1. **Review Provider Options**: Explore the available LLM providers in the [LLM Config](/admin/llm_providers) section
2. **Set Up Credentials**: Configure API keys and authentication for your chosen providers
3. **Test Models**: Experiment with different models to find the best fit for your use cases
4. **Configure User Access**: Set up appropriate permissions and default models for users
5. **Monitor and Optimize**: Track usage patterns and adjust configurations as needed
