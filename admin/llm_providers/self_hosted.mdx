...
Self-hosted LLMs allow you to run AI models on your own infrastructure, providing complete control over data, security,
and costs. This guide will help you configure self-hosted models in Onyx.
...
- **Infrastructure**: Sufficient compute resources (GPU/CPU)
- **Model Files**: Downloaded model weights and configuration
- **Inference Server**: Ollama, vLLM, or similar inference engine
- **Network Access**: Proper network configuration for Onyx to reach the model
- **Monitoring**: Tools to monitor model performance and resource usage
...
<Steps>
  ...
  <Step title="Select Self-Hosted">
    Go to **LLM Config** > **Self-Hosted**
  </Step>

  <Step title="Choose Ollama">
    Select **Ollama** as the inference server
  </Step>
  ...
  <Step title="Choose model">
    Choose the model name (e.g., `llama2:7b`)
  </Step>
...
<Steps>
  ...
  <Step title="Choose vLLM">
    Select **vLLM** as the inference server
  </Step>

  <Step title="Enter server URL">
    Enter the vLLM server URL (e.g., `http://localhost:8000`)
  </Step>
...
<Steps>
  ...
  <Step title="Choose Custom">
    Select **Custom** as the inference server
  </Step>
  ...
  <Step title="Configure API format">
    Configure the API format and parameters
  </Step>
</Steps>
...
- **Firewall Rules**: Restrict access to inference servers
...
- **GPU Optimization**: Use appropriate GPU drivers and CUDA versions
- **Memory Management**: Optimize memory allocation and usage
- **Network Optimization**: Use high-speed network connections
- **Storage Optimization**: Use fast storage for model files
...
- **Optimize Configuration**: Fine-tune parameters for your use cases
- **Train Users**: Educate users on model capabilities and limitations
- **Scale Deployment**: Gradually increase usage while monitoring performance

For additional support or questions about self-hosted LLM integration,
refer to the documentation for your chosen inference server or contact the Onyx support team.
