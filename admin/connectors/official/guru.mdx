---
title: Guru
description: "Access cards and collections from Guru"
---

## How it works
The Guru connector pulls in all the Cards your user has access to based on a User Access Token.

Cards are updated every **10** minutes.


## Setting up
### Authorization
    1. Obtain a User Access Token.
    - Follow the Guru API guide described [here](https://developer.getguru.com/docs/getting-started) for more details.

![GuruConnectorAuth](/assets/images/connectors/guru/GuruConnectorAuth.gif)

### Indexing
    1. Navigate to the Admin Panel and select the **Guru** Connector
    2. In **Step 1**, provide your **Username** (most likely your email) and the **Access Token** from above.

![GuruConnector](/assets/images/connectors/guru/GuruConnector.png)

    3. Click `Connect` to begin indexing
