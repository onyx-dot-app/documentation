...
<AccordionGroup>
  <Accordion title="Agents" icon="user">
    ...
    **Built-in Agents:**
    ...
    You can create your own Agents in the Admin Dashboard or by API.

    **Most Chat endpoints require an Agent ID**

    To find your Agent ID, you can:
    - Use the `GET /persona` API endpoint to list all agents
    - In the Admin Dashboard: Click into an agent and check the first number in the URL
  </Accordion>
</AccordionGroup>
...
## Next Steps
<CardGroup cols={2}>
  <Card title="Guide: Create a Connector with the API" icon="lightbulb" href="/developers/guides/create_connector_api">
    Learn how to create a Connector through the API
  </Card>
  
  <Card title="Guide: Set up an MCP (Admin Dashboard)" icon="comment" href="/developers/guides/mcp_guide">
    Learn how to set up an MCP server in the Admin Dashboard
  </Card>
</CardGroup>