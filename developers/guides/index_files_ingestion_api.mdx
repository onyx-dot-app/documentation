...
<Info>
  **Required permission:** Manage Connectors & Document Sets (`manage:connectors`), or Admin (`admin`).
  A **Group Manager** can ingest into connector-credential pairs in the groups they manage,
  but not into the default pair, which is public.
</Info>
...
<Steps>
  <Step title="Prepare your Connector">
    ...
    `https://cloud.onyx.app/admin/connector/308` -> `cc_pair_id` is `243`
  </Step>
    ...
    <CodeGroup>
      ...
      cat > document_payload.json << 'EOF'
      {
        "document": {
          ...
          "sections": [
            {
              "text": "This is the contents of the document that will be processed and saved into the Onyx filestore and OpenSearch index.",
