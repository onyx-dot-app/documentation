...
- A model catalog limited to the models that your Onyx identity can access.
...
Create a PAT from **Settings** > **Accounts & Access**. Choose **Limited access** and select **LLM Gateway**.
Treat the token as a password. Onyx shows it only once.
...
Call `GET /models` before you configure a client.
The response contains only models that are visible and accessible to your account.

```bash
curl "$ONYX_GATEWAY_URL/models" \
  -H "Authorization: Bearer $ONYX_GATEWAY_API_KEY"
```

Use the returned `id` exactly as the `model` value. Gateway model IDs use this format:

```text
<provider-id>/<model-name>
```

For example: `12/gpt-5-mini`.

The catalog also reports each model's input modalities, reasoning support,
and input and output token limits when Onyx knows them.
...
Set the gateway's parent endpoint as the Anthropic base URL, then use a Claude model ID returned by `GET /models`:
...
Use model IDs from your own catalog; provider IDs differ by deployment.
...
The configured model must be an OpenAI-compatible model ID returned by `GET /models`.
...
Replace the example model with one from your gateway catalog.
...
| Model not found | Call `GET /models` and use a returned model ID exactly. The provider ID is part of the identifier. |
