...
<Note> This API will be out in version 2.8 and is not live yet </Note>

## POST `/chat/send-message-non-streaming`
<Tip>
  There is a streaming version of the endpoint `/chat/send-message` which gives back fine grained packets such as the reasoning
  tokens, answer tokens, and tool calls/responses as they happen. The caller must reconstruct the reasoning, tool calls and
  answer from these packets.
</Tip>
...
<ParamField body="chat_session_id" type="integer">
  The ID of chat session where the message is sent. To send follow ups in a conversation, specify the chat session where the message
...
<ParamField body="chat_session_info" type="object">
  Specify details about the chat session which will be used for all messages in the session. This is ignored if an existing
  chat_session_id is provided instead. It can also be left blank to use the default settings.
...
<ParamField body="deep_research" type="boolean">
  Turn on to use the Deep Research flow. Note that this mode is a lot more token intensive so be careful if accessing it programmatically.
</ParamField>
...
- Tool calls
- Tool call responses
- Any intermediate reasonings
- Any referenced documents
- Final LLM response to the user query
