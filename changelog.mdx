...
<Update label="v3.0.0" description="March 9th, 2026" tags={["New Features", "Deployment Changes", "Bug Fixes"]}>
  ...
  Panel, and we've passed Okta's Runscope spec test for OIN submission.
  ...
  by default on the default agent.
  ...
  Memory has been rewritten with ID-based persistence and a redesigned UI for
  managing what your agent remembers. Agents now read structured user context
  directly from the system prompt, and the memory tool can add or update
  memories during a conversation.

  ### Image Generation Edits with Historical Context

  OpenAI, Azure, and Vertex AI image generation now support editing images using
  ...
  - New local file connector for Craft
  - Slab connector validation
  - Confluence and Google Drive connectors are now backed by hierarchy nodes for
    more accurate pruning

  ### Craft and Artifacts

  - PPTX generation, editing, and preview
  - Improved PDF preview and a refreshed output panel
  - Sandbox sessions persist across sleep/wake
  - Shareable webapp URLs with two-tier access control
  - Output files are downloadable from the Artifacts tab
  - S3 snapshot files are cleaned up when a craft is deleted
  ...
  - Seat limits are enforced on every user-creation path, including bulk invite
    and Slack
  ...
  - Public OAuth clients are now supported via Dynamic Client Registration

  ### Deployment, Helm, and Observability

  - Production Prometheus instrumentation for the API server, including
    SQLAlchemy connection pool and per-tenant metrics
  - OpenSearch is included in the default `docker-compose.yml` and Helm chart
  - Code Interpreter is included in the default Docker and Helm deployments
  - Dedicated Celery migration queue in the Helm chart
  - KEDA `idleReplicaCount` is now optional and fallbacks apply only on custom
    metrics
  - nginx routes `/openapi.json` to the API server correctly
  - EE license public key is now baked into the Docker image
  - New Postgres-backed cache backend abstraction for caches that previously
    required Redis
  - Background tasks can run via FastAPI in addition to Celery
  ...
  - Fixed Ollama context-window passthrough, content skipping, and DeepSeek
    handling
  - Fixed Anthropic model selection under multi-tenant
