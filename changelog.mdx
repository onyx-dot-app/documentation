...
<Update label="v4.8.0" description="September 18th, 2026" tags={["New Features", "Deployment Changes", "Bug Fixes"]}>
  ...
  Outlook joins the connector lineup with mail, calendar, attachments, and permission sync.
  ...
  - **`onyx-cli deploy upgrade` finishes the job it started.** The bundled nginx resolves each service address
    once at startup,
    so after an upgrade replaced the API server it kept proxying to the old container's address and served 502s until
    its next restart (in practice, hours). Upgrades now reload the proxy after the new containers are up. Also fixed:
    `--tag vX.Y.Z-dev` image tags fetched their compose configuration from a git ref that does not exist and silently
    fell back to the files baked into the CLI binary; `-dev` tags now use their plain tag's configuration,
    and an invalid tag fails before anything is written.
  ...
  Cancelled and private/confidential events are never indexed. With permission sync enabled (Enterprise),
  ...
  After the switchover, the old index is held for a grace period while the new one proves healthy,
  then is deleted automatically with the deletion verified against document counts,
