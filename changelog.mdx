...
<Update label="v4.4.0" description="July 21st, 2026" tags={["New Features", "Deployment Changes", "Bug Fixes"]}>
  ...
  SSO configuration has been rebuilt from the ground up. Google, OIDC,
  and SAML providers are managed from a new admin page — create, edit, and enable them without redeploying,
  ...
  - Fixed v4.3 Helm chart deploy blockers (sandbox-proxy IPv6 network
    policy and an nginx crash-loop).
  ...
  - Fixed the empty OAuth credential modal for zero-kwargs OAuth
    connectors.
  - Fixed multi-IdP SSO logins failing to claim permission-sync
    placeholder users.
