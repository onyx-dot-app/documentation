...
<Update label="v4.7.0" description="September 4th, 2026" tags={["New Features", "Deployment Changes", "Bug Fixes"]}>
  ...
  - Comparing models side by side no longer interrupts the conversation: sending after a multi-model turn just
    works - an explicit response pick wins, otherwise the previous turn's model (or the first) carries the
    thread.
  - Multi-model turns are easier to read and pick from: narrow screens get a swipeable single-response carousel,
    panel headers stay pinned with the model name while long responses scroll, the whole card is the selection
    target, and the Retry button attributes each response to the model that actually produced it.
