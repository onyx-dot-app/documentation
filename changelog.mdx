...
<Update label="v4.7.0" description="September 4th, 2026" tags={["New Features", "Deployment Changes", "Bug Fixes"]}>
  ...
  and an entire deployment becomes expressible as code: a new Terraform provider configures the application itself,
  alongside refreshed AWS modules and a brand-new Azure module set. Multi-model chat loses its friction,
  Salesforce gains OAuth, and workspaces on Onyx Cloud can bring their own SSO.
  ...
  ### Manage Onyx with Terraform

  The new [Terraform provider](https://github.com/onyx-dot-app/onyx/tree/main/terraform-provider-onyx)
  makes Onyx configuration declarative: workspace settings, LLM providers and the default model, the embedding provider,
  API keys, credentials, connectors, connector-credential pairs, document sets, agents, custom tools, MCP servers,
  and user groups are all expressible - an entire indexing and assistant setup can be reviewed, versioned, and applied.
  Every secret can be supplied through write-only attributes that never land in Terraform state (Terraform 1.11+),
  masked reads are handled correctly, and the client retries transient failures without ever replaying a create.
  The provider ships with a day-one bootstrap example and an acceptance suite that runs against a real Onyx stack in CI.

  ### Multi-Model Chat Without the Friction

  Comparing models side by side no longer interrupts the conversation:
  sending after a multi-model turn just works - an explicit response pick always wins,
  otherwise the previous turn's model (or the first) carries the thread.
  Narrow screens get a swipeable single-response carousel,
  panel headers stay pinned with the model name while long responses scroll, the entire card is the selection target,
  and the Retry button attributes each response to the model that actually produced it.
  Admins gain per-model controls in the LLM provider modal - a reasoning-effort cap, a default reasoning level,
  and a default temperature,
  enforced at inference time on every path so no client can exceed the cap - and each user can set personal defaults for
  temperature and reasoning under Settings -> Chat Preferences.
  A new workspace setting can flatten the model selector into a single ungrouped list,
  for deployments that route everything through a gateway.

  ### Bring Your Own SSO on Onyx Cloud

  Workspaces on Onyx Cloud can now configure their own Google or OIDC single sign-on.
  Admins prove domain ownership with a DNS TXT record,
  and verified domains route sign-ins from the shared cloud login page straight to the right workspace - including
  first-time users, who are provisioned just in time on a verified domain. Allowed email domains are required,
  so an IdP can never assert its way into unbilled seats.
