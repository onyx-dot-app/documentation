...
<Update label="v3.1.0" description="April 1st, 2026" tags={["New Features", "Bug Fixes", "Performance"]}>
  ...
  - **Agents are private by default.** Newly-created agents are no longer
    shared with everyone in the deployment unless the creator explicitly
  ...
  - Configurable file upload size limit, with a frontend precheck for
    oversized uploads
  ...
  - jemalloc replaces glibc malloc in the API server, reducing memory
    fragmentation
  ...
  - Fixed Vespa ACL query failures by using `weightedSet` for filters
  - Fixed assistant file transfer
  - Fixed default model removal/hiding edge cases
  - Fixed Craft webapp asset and HMR reload leaks, and a route conflict in the
    webapp download endpoint
