---
title: "EKS with Helm Chart"
description: "Use Onyx Helm Chart to Setup on AWS EKS"
---
...
- Download and install [helm  CLI](https://helm.sh/docs/helm/helm_install/). This is required for installing onyx helm chart.
...
Follow the process for creating the access key and secret. Select the `Command Line Interface (CLI)` option during creation. Be sure to save the `Access key` and `Secret access key` for later.
...
## Deploy Onyx Services to EKS Cluster
...
Rebuild the charts based on the state specified in the Chart.lock file.
...
Additional material found on the [Helm documentation](https://helm.sh/docs/).

### Monitor Onyx Services
