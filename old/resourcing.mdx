...
`vespa` - >=4 CPU, >= 8Gi Memory. This is the bare minimum for a production deployment, and we would generally recommend
...
tuning the `VESPA_SEARCHER_THREADS` environment variable. See the [How Resource Requirements Scale](#how-resource-requirements-scale) 
