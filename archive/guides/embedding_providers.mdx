...
Our application supports integration with several popular embedding model providers,
offering you flexibility in choosing the model that best suits your needs. Here's an overview of the available models:
...
3. Use your Cohere API key in Onyx.
...
3. Use your Voyage AI API key in Onyx.
...
6. Use the JSON key file in Onyx by uploading your file.
...
3. **Performance vs. Efficiency**:
   Balance the trade-off between model performance and computational efficiency based on your needs.
4. **Integration Complexity**: Consider the ease of setup,
   especially for models like Gecko that require more configuration.
...
4. **Security**:
   Always use the secure interface provided by the application to input your API keys or upload credential files.

Remember to review each provider's documentation for the most up-to-date information on model capabilities and
integration details.
