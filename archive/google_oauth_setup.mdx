...
![GoogleOAuthConsentMenu](/archive/images/google_oauth_setup/GoogleOAuthConsentMenu.png)
...
![GoogleApp](/archive/images/google_oauth_setup/GoogleApp.png)
...
OAuth is controlled by 3 environment variables, regardless of deployment choice (non-containerized, docker compose,
kubernetes). To turn the feature on set:
...
- The frontend hits the api server to determine what user authentication setting is configured and serves the relevant
  pages.
...
on by default. To set up the required values,
replace the `REPLACE-THIS` values in secrets.yaml with thebase64 encoded client ID and client secret from above.
