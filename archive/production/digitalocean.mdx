...
This is a simple and cost effective way to deploy Onyx and should meet the performance needs of smaller organizations
looking to use Onyx.
...
Next, point your domain to the Droplet. To do this,
you'll need to add two records to your DNS and set your Nameservers to Digital Ocean's.
...
![Instance](/archive/images/setup_guides/do/InstanceIp.png)

Finally, we need to head to the DNS provider and add two entries into the DNS:

![Instance](/archive/images/setup_guides/do/ARecord.png)

![Instance](/archive/images/setup_guides/do/CnameRecord.png)

The first record directs traffic to that domain to your Droplet.
The second record will handle `www.<YOUR_DOMAIN>` and ensure that this also takes the user to your container running on
your Droplet.
...
Once the Droplet agent is installed (and configured if necessary),
you can access the Droplet Console from the DigitalOcean Control Panel. First,
click the name of the Droplet you want to access to expand its detail window,
then click the name again to go to its detail page. From the detail page, click the Access tab in the left menu.

In the Droplet Console section, enter the user you want to log in as in the Log in as… field. By default,
this is set to root. Once you choose a user, click Launch Droplet Console to open the Droplet console.
