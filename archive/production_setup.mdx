...
Below, we discuss the simplest possible way to setup Onyx within your enterprise.
We know that every company does things differently,
and we'd love to help you figure out the ideal setup for your enterprise!
Feel free to reach out to us via the links on our [Contact Us](https://docs.onyx.app/contact_us) page.
...
For resourcing, we recommend a machine with 8vCPU and 16GB memory (although 4vCPU and 8GB memory can work in a pinch).
    ...
    - Setup your `.env` files
      ...
      - Make a copy of `env.nginx.template` called `.env.nginx` (`cp env.nginx.template .env.nginx`)
        - Fill in the `DOMAIN` variable with the domain that people will use internally to access the Onyx UI - **NOTE:** this domain must be publicly accessible for this flow to work. If you're hosting it internally and
         not exposing it to the public internet then feel free to reach out to us to figure out the best way to set this
         up.
        - `EMAIL` is optional
