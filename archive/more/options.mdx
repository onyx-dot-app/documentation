...
If you must delete a connector and related information before the process of waiting for the background jobs to run,
you can run a `deletion` script using the instructions that follow.
...
![Connectors page](/archive/images/more/connectors.png)

2. Click on the connector you want the ID of .

![Connector ID](/archive/images/more/connectorID.png)

3. On the connector's page, the URL's final number will be the ID of the connector. For example, in this example,
   the connector's ID is 2. Here is also a more zoomed in photo of the URL bar.

![Zoomed in Connector ID](/archive/images/more/zoomConnectorID.png)
...
1. Ensure your Docker containers are running- you can refer to the [quickstart guide](/archive/quickstart):
...
4. Run the deletion script inside the Docker container:
   ...
   Replace `<CONTAINER_ID>` with your actual API container ID,
   and `<CONNECTOR_ID>` with the ID of the connector you want to delete.
