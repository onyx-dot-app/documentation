...
Onyx creates one document for each record of that boject. If you pick none, Onyx indexes **Account**.
...
<Columns cols={2}>
  <Card title="OAuth (self-hosted Onyx only)" icon="key" href="#authenticate-with-oauth">
...
<Info>
  If you plan to use a username and password,
  skip to [Authenticate with a security token](#authenticate-with-a-security-token).
...
<AccordionGroup>
  ...
  <Accordion title="The OAuth option is missing from the credential form">
    On Onyx Cloud, OAuth is not available yet. Use a username and password instead.
