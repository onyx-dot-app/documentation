...
Slack seems to have softened their stance on API rate limits, which allows us the standard approach to work again.
We strongly recommend going that direction if it's an option for better search quality until the [Assistant Search](https://docs.slack.dev/reference/methods/assistant.search.info/)
API is publically released.
