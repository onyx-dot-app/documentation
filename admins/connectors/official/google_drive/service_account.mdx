...
<Note>
  You can create more than one Google Drive credential on a single Onyx instance, each with its own
  service account. This lets you index multiple Google Workspaces from one Onyx deployment. Create a
  separate credential and connector for each workspace you want to index.
</Note>
