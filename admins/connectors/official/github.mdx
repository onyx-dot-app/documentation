...
#### Permission Sync Setup (Fine-Grain Access Token)

When creating a **fine-grained** GitHub access token for **permission sync**, make sure you select the correct
**Resource owner organization** (the GitHub **organization** that owns the repositories you want to create the connector for).
Fine-grained tokens scoped to a personal user account do **not** allow granting **Organization permissions** (like **Members**),
so permission sync requires selecting an organization as the resource owner.
If you select the wrong organization, the token will not be able to access the intended org/repositories and permission sync will fail.


For permission sync functionality, you also need additional permissions. Add these in the token configuration page by using
**Repository permissions** -> **Add permission** and **Organization permissions** -> **Add permission**:
...
<Steps>
  <Step title="Repository permissions">
    Add the following with **Read access**:
    - **Administration**
    - **Issues**
    - **Metadata**
    - **Pull requests**
  ...
  <Step title="Organization permissions">
    Add the following with **Read only**:
    - **Administration**
    - **Members**
  </Step>
</Steps>

#### Permission Sync Setup (Classic Token)

If you're using a **classic** GitHub personal access token (PAT) and want **permission sync**, you must also grant the
organization read scope so Onyx can enumerate org membership:
