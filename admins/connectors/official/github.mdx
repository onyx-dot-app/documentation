  ...
  <Tab title="Fine-grained access token">
    <Steps>
      ...
      <Step title="Add required permissions">
        Add these in the token configuration page using **Repository permissions** -> **Add permission**
        and **Organization permissions** -> **Add permission**:
  ...
  <Tab title="Classic token">
    <Steps>
      <Step title="Add the org read scope">
        If you're using a **classic** GitHub personal access token (PAT) and want **permission sync**, you must also grant:
        - `read:org`
      </Step>
