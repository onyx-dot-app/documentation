...
failed ones included, or one million characters of attachment text. Set `MICROSOFT_CONNECTOR_SIZE_THRESHOLD` (bytes)
to change the 20 MB (20 × 1024 × 1024 bytes) size limit for the SharePoint, Teams and Outlook connectors together,
or `OUTLOOK_CONNECTOR_ATTACHMENT_SIZE_THRESHOLD` to change it for Outlook alone.
