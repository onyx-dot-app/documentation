...
description: "Configure Craft sandboxes for Docker Compose and Kubernetes deployments"
icon: "hammer"
---
...
<Warning>
  `local` is not a supported Craft sandbox backend. Use `docker` or `kubernetes`.
</Warning>
...
Manual setup is easy to get wrong — prefer `./install.sh --include-craft`. If you manage compose files yourself,
...
<Warning>
  ...
  The install script prints a reminder,
  but it does **not** install a host-level `DOCKER-USER` iptables rule for you — add one yourself if your host exposes
  instance metadata.
</Warning>
...
the chart rejects any `SANDBOX_BACKEND` other than `kubernetes` — the Docker backend is not available in Helm
deployments.
...
<Note>
  ...
  and Helm Craft follows the chart/app version — override only when intentionally testing a custom sandbox image with
  support guidance.
</Note>
