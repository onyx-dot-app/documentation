...
that provision the AWS infrastructure Onyx needs: VPC, EKS, RDS for PostgreSQL,
ElastiCache for Redis, S3, and (optionally) OpenSearch and WAF. Once the infra
is up, you install the Onyx workloads with the [Helm chart](/deployment/local/kubernetes).
...
The composed `onyx` module wires together the building blocks below. You can
...
<Steps>
  ...
  <Step title="Clone the Onyx repo">
    ...
    The modules live under `modules/aws/`. You will create a small root module
    that calls them.
  </Step>

  <Step title="Pick an AWS region and a name">
    The `name` variable is used as a prefix for everything the modules create
    (VPC, cluster, RDS, S3 bucket, etc.) and is combined with the current
    Terraform workspace, so the same code can produce isolated `dev`, `staging`,
...
The snippet below is a minimal root module that provisions a complete Onyx
stack on AWS using the composed `onyx` module.
...
The full apply takes ~20 minutes — most of the time is spent waiting on EKS,
RDS, and ElastiCache.
...
<ParamField path="postgres_password" type="string" required>
  ...
  `TF_VAR_postgres_password` or your secrets manager rather than hard-coding.
</ParamField>
...
<ParamField path="create_vpc" type="bool" default="true">
  When `true`, the module creates a new VPC sized for EKS. Set to `false` to
  reuse an existing VPC — see [Using an existing VPC](#using-an-existing-vpc).
</ParamField>

<ParamField path="public_cluster_enabled" type="bool" default="true">
  Whether the EKS API endpoint is reachable from the public internet. Combine
  with `cluster_endpoint_public_access_cidrs` to lock it down to specific IPs.
</ParamField>

<ParamField path="private_cluster_enabled" type="bool" default="false">
  Enable the private EKS API endpoint. Recommended for production. You can
...
OpenSearch is **off by default**. Enable it if you want a managed search
backend instead of the in-cluster Vespa node group.
...
The `waf` module is optional but **strongly recommended** for any
...
For the full list of inputs, see [`modules/aws/onyx/variables.tf`](https://github.com/onyx-dot-app/onyx/blob/main/deployment/terraform/modules/aws/onyx/variables.tf).

## Using an existing VPC

If you already have a VPC you want to deploy into, set `create_vpc = false`
...
<Warning>
  The S3 bucket policy locks bucket access to the provided
  `s3_vpc_endpoint_id`. Make sure your worker nodes route S3 traffic through
  that endpoint, otherwise S3 calls from the cluster will be denied.
</Warning>

## Production hardening

For production environments we recommend the following deltas from the
quickstart:
...
Once `terraform apply` finishes, the `onyx` module exposes the values you'll
...
Terraform only stands up the infrastructure — Onyx itself is installed via
the [Helm chart](/deployment/local/kubernetes).
...
<Steps>
  ...
  <Step title="Install the chart">
    The EKS module creates an IRSA-backed `ServiceAccount` named
    ...
    in-cluster MinIO so file storage uses the real S3 bucket.
    ...
    See the [Kubernetes deployment guide](/deployment/local/kubernetes) for
...
Every resource the `onyx` module creates is named with the active Terraform
...
A `name = "onyx"` module call in workspace `prod` will produce
...
- **Sensitive outputs.** `postgres_username`, `redis_connection_url`, and the
  EKS CA data are marked sensitive. Hand them off to your secrets manager or
  Helm values file rather than echoing them to logs.
...
- **First apply is infra-only.** EKS takes several minutes to become active.
  The `null_resource.wait_for_cluster` block in the quickstart blocks the
  Kubernetes/Helm providers until the API server is reachable.
- **The Vespa node group is tainted.** The `eks` module provisions a
  dedicated node group for Vespa with a `vespa-dedicated=true:NoSchedule`
  taint. The Helm chart's Vespa pods tolerate it; everything else lands on
  the main node group.
