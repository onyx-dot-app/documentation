...
- **The OpenSearch node group is tainted.** The `eks` module provisions a
  dedicated node group for OpenSearch with a `vespa-dedicated=true:NoSchedule` taint.
  The Helm chart's OpenSearch pods tolerate it; everything else lands on the main node group.
