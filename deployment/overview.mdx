...
**Onyx Lite** — A lightweight Chat UI that uses fewer resources than Onyx Standard.
It is great for users who want to try Onyx quickly or teams only interested in Chat and Agents. Includes LLM chat,
tools, file uploads, and Projects, but does not use the vector database or background workers.
Docker Compose Lite skips both model servers;
Helm Lite still starts the inference model server unless you disable it explicitly.

<Note>
  Onyx Lite does not include indexing. Use Onyx Standard to index knowledge from connected applications.
</Note>

## Quickstart

<Card title="Install Script (Recommended)" icon="bolt" href="/deployment/getting_started/quickstart">
  A guided installation script that deploys Onyx in Docker Compose. Supports both Standard and Lite modes.
</Card>
...
<CardGroup cols={1}>
  <Card title="Self-Host Open Source" icon="code" href="/deployment/getting_started/quickstart">
    Get all of Onyx's core features, including Internal Search, Connectors, Chat, Agents, Actions, Deep Research,
    and more! All of your data remains secure within your Onyx deployment.
