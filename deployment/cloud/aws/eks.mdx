...
<Tip>
  To create the EKS cluster, RDS, ElastiCache, and S3 with code,
  use the [Terraform modules](/deployment/local/terraform).
</Tip>
...
<Steps>
  ...
  <Step title="Install Onyx services">
    ...
    into a single deployment. By default, persistent volumes will be created for stateful services.
...
```bash
helm upgrade onyx onyx/onyx -n onyx --version <VERSION>
```
...
OpenSearch, Postgres, and MinIO leave behind PVCs. To delete them:

```bash
kubectl -n onyx get pvc
kubectl -n onyx delete pvc onyx-opensearch-master-onyx-opensearch-master-0
