...
description: "Onyx configuration options"
icon: "sliders"
---
...
Open source Onyx supports basic email and password authentication and Google oauth

Onyx Enterprise Edition adds OIDC, and SAML support.

See [Authentication](/deployment/authentication) for more information.

### Custom Domain

To host Onyx on a custom domain, set the `WEB_DOMAIN` environment variable.

For additional instructions, see [EC2 Deployment](/deployment/deployment_guides/aws/ec2) or [EKS Deployment](/deployment/deployment_guides/aws/eks) for docker and kubernetes deployments respectively.

### Internet Search

To enable the Internet Search Tool, set the `EXA_API_KEY` environment variable.

See [Internet Search](/admin/tools/internet_search) for more information.

### Logging

Set `LOG_LEVEL` to `debug` in each container for detailed logging. This is helpful when troubleshooting issues.
