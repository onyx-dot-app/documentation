...
- **OpenSearch** (vector database): no document indexing or RAG search
...
|----------------------|--------------|-------------------|----------------------------------------------|
| `DISABLE_VECTOR_DB`  | `true`       | `false`           | Disables OpenSearch, connectors, and RAG search   |
...
### Deploying Onyx Lite with Docker Compose
    ...
    The override moves non-essential services (OpenSearch, Redis, model servers, background workers)
    ...
    **Quick start:**
    ...
    | Service | Profile | Start container with |
    |--------------------------|----------------|-----------------------------------------|
    | background (Celery) | background | --profile background (also needs --profile redis) |
    | cache (Redis) | redis | --profile redis |
    | index (OpenSearch) | opensearch | --profile opensearch |
    | indexing_model_server | vectordb | --profile vectordb |
    | inference_model_server | inference | --profile inference |

    Starting the OpenSearch or indexing model server containers does not enable indexing in Lite.
    The API server still has `DISABLE_VECTOR_DB=true`; use a full Onyx deployment for indexing and RAG search.

    <Tip>
      You can selectively start containers by adding `--profile <name>` flags. For example, to run Lite with Redis:
