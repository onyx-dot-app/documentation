...
<AccordionGroup>
  ...
  <Accordion title="Connector Configuration">
    ...
    **Microsoft 365 Connectors**
    ...
    `MICROSOFT_CONNECTOR_SIZE_THRESHOLD`: File size threshold in bytes for SharePoint, Teams and Outlook (default 20 MB)

    `SHAREPOINT_CONNECTOR_SIZE_THRESHOLD`: File size threshold for SharePoint, overrides the shared value

    `TEAMS_CONNECTOR_ATTACHMENT_SIZE_THRESHOLD`: File size threshold for Teams, overrides the shared value

    `OUTLOOK_CONNECTOR_ATTACHMENT_SIZE_THRESHOLD`: Attachment size threshold for Outlook, overrides the shared value
