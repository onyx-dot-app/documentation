...
<AccordionGroup>
  ...
  <Accordion title="Configuring Kubernetes Deployments">
    ...
    Add environment variables and secrets to the `configMap` and `auth.secrets` sections. A subset of common configuration options are provided for you.
    You can add more environment variables (listed below) to these sections.
  </Accordion>
</AccordionGroup>

<Tip>
  Many configuration options are available in the Admin Dashboard! This page only covers deployment-level configuration.
</Tip>
...
For additional instructions, read the [EC2 Deployment Guide](/deployment/deployment_guides/aws/ec2)!
...
To enable the Internet Search Tool, set the `EXA_API_KEY` environment variable.

<Note>
  Internet Search in Onyx is continually developing and these instructions will be updated as we make improvements.
</Note>
...
Although Onyx assumes English by default, the system can be configured to support multiple languages in the Admin Dashboard and through environment variables.

1. Select a multi-lingual embedding model in the **Search Settings** of the Admin Dashboard
...
<AccordionGroup>
  <Accordion title="ENV_SEED_CONFIGURATION">
    Setting this variable allows you to start your Onyx instance with pre-configured options that persist across restarts.
