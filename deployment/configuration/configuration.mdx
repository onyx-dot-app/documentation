...
- Tools (including code interpreter when its service is enabled)
- User file uploads
- Projects
- Agent knowledge
...
Onyx Lite disables or does not use:
- **Vector indexing and RAG search**: `DISABLE_VECTOR_DB=true` prevents use of OpenSearch, even if it is running
- **Redis**: PostgreSQL handles caching and session management instead
- **Indexing model server**: no document embeddings; Docker Compose Lite also skips the inference model server
- **Celery background workers**: the API server handles background work directly
- **MinIO / S3 file storage**: PostgreSQL handles file storage instead, even if MinIO is running
- **Connectors**: all data source connectors are disabled

Helm's Lite values leave the OpenSearch, MinIO, inference model server, code interpreter,
and Slack bot workloads enabled by chart defaults.
The unused OpenSearch and MinIO workloads do not enable indexing or S3 file storage.

### How It Works

Onyx Lite is not a separate application. It uses the same Onyx images with a configuration that stores files,
cache data, and sessions in PostgreSQL and disables vector database access.
The key environment variables that drive this are:

| Variable             | Lite Value   | Full Onyx Default | Purpose                                      |
|----------------------|--------------|-------------------|----------------------------------------------|
| `DISABLE_VECTOR_DB`  | `true`       | `false`           | Disables vector indexing, connectors, and RAG search |
...
### Deploying Onyx Lite with Docker Compose
    ...
    The override moves unused services into Docker Compose [profiles](https://docs.docker.com/compose/how-tos/profiles/)
    ...
    **Quick start:**
    ...
    | Service | Description |
    |------------------|-----------------------------------|
    | api_server | Onyx API server (also handles background work) |
    | web_server | Next.js frontend |
    | relational_db | PostgreSQL database |
    | nginx | Reverse proxy |
    | code-interpreter | Python tool service |

    <Note>
      The .env file setup is the same as a standard Docker deployment.
      Lite-specific environment variables (DISABLE_VECTOR_DB, CACHE_BACKEND, AUTH_BACKEND, FILE_STORE_BACKEND)
      are set automatically by the override file, so you do not need to add them to your .env.
    </Note>
  </Accordion>
...
### Deploying Onyx Lite with Helm
    ...
    The values-lite.yaml file disables vector database access, Redis, Celery workers, and the indexing model server.
    Unlike the Docker Compose overlay, it does not disable OpenSearch, MinIO, the inference model server,
    or the Slack bot. These workloads still start with the chart defaults. The code interpreter also remains enabled.

    For a minimal install that omits those unused workloads, add explicit overrides:
    ...
    <Tip>
      To upgrade from Lite to a full deployment,
      use the chart defaults without values-lite.yaml or the minimal-install overrides. This enables vector indexing,
...
Craft is enabled with `ENABLE_CRAFT=true` and requires a supported sandbox backend.
Use `SANDBOX_BACKEND=docker` for Docker Compose deployments and `SANDBOX_BACKEND=kubernetes` for Helm deployments.

Read [Deploying Craft](/deployment/local/craft) before enabling Craft in self-hosted deployments.
...
<AccordionGroup>
  ...
  <Accordion title="Craft Configuration">
    ...
    Docker Compose Craft uses the `onyx_craft_sandbox` bridge network created by the installer and referenced by the
    Craft compose overlay.
  </Accordion>
  ...
  <Accordion title="OpenSearch Configuration">
    `OPENSEARCH_HOST`: OpenSearch host

    `OPENSEARCH_REST_API_PORT`: OpenSearch REST API port (default `9200`)

    `OPENSEARCH_ADMIN_USERNAME`: OpenSearch admin username

    `OPENSEARCH_ADMIN_PASSWORD`: OpenSearch admin password

    `OPENSEARCH_USE_SSL`: Use TLS when connecting to OpenSearch (default `true`)

    `OPENSEARCH_VERIFY_CERTS`: Verify the OpenSearch server certificate

    `OPENSEARCH_CA_CERTS`: CA bundle used to verify the server certificate

    `OPENSEARCH_CLIENT_CERT`: Client certificate for mutual TLS

    `OPENSEARCH_CLIENT_KEY`: Client key for mutual TLS

    `OPENSEARCH_AUTH_METHOD`: `basic` (default) or `iam` (AWS SigV4 signing for managed domains)

    `OPENSEARCH_AWS_REGION`: AWS region of the managed OpenSearch domain (required for `iam`)

    `OPENSEARCH_AWS_SERVICE`: SigV4 service name, `es` (default) or `aoss` for OpenSearch Serverless

    `USING_AWS_MANAGED_OPENSEARCH`: Set to `true` when using an AWS managed OpenSearch domain

    `DEFAULT_OPENSEARCH_CLIENT_TIMEOUT_S`: Client request timeout in seconds

    `DEFAULT_OPENSEARCH_QUERY_TIMEOUT_S`: Query timeout in seconds

    `OPENSEARCH_TEXT_ANALYZER`: Text analyzer for keyword search (default `english`)

    `OPENSEARCH_INDEX_NUM_SHARDS`: Number of primary shards per index

    `OPENSEARCH_INDEX_NUM_REPLICAS`: Number of replicas per index

    `PIT_KEEP_ALIVE`: Point-in-time keep-alive used when scanning an index (default `5m`)

    `OPENSEARCH_PROFILING_DISABLED`: Disable OpenSearch query profiling (default `true`)

    `OPENSEARCH_MATCH_HIGHLIGHTS_DISABLED`: Disable match highlights in search results (default `true`)

    `OPENSEARCH_EXPLAIN_ENABLED`: Return per-hit score breakdowns for relevance debugging (large performance cost)

    `ONYX_SEARCH_UI_USES_OPENSEARCH_KEYWORD_SEARCH`: Use OpenSearch keyword search for the Search UI

    `ENABLE_OPENSEARCH_INDEXING_FOR_ONYX`: Index documents into OpenSearch (default `true`)

    `ONYX_DISABLE_VESPA`: Run without the legacy Vespa index (default `true`)

    `DISABLE_OPENSEARCH_MIGRATION_TASK`: Disable the periodic Vespa to OpenSearch migration task
  </Accordion>
  ...
  <Accordion title="Celery Configuration">
    ...
    `CELERY_PERMISSIONS_SYNC_LOCK_TIMEOUT`: Per-connector document permission sync lock TTL in seconds.
    Defaults to `3600`.
    Raise it if a permission sync on a large tenant fails with "Cannot reacquire a lock that's no longer owned".

    `CELERY_DOCUMENT_SYNC_BEAT_LOCK_TIMEOUT`: Document-index sync lock TTL in seconds. Defaults to `120`.
    Raise it if the sync pass aborts with "no longer owned" or "Lock not owned on completion".
  </Accordion>
  ...
  <Accordion title="Logging Configuration">
    ...
    `LOG_VESPA_TIMING_INFORMATION`: Log document index query performance
  ...
  <Accordion title="File Storage Configuration">
    `FILE_STORE_BACKEND`: File store backend. Use `s3` (default) for AWS S3, MinIO, and other S3-compatible storage,
    `gcs` for Google Cloud Storage, `azure` for Azure Blob Storage, or `postgres` to keep files in PostgreSQL.

    **S3 and MinIO** (`FILE_STORE_BACKEND=s3`)
    ...
    **Google Cloud Storage** (`FILE_STORE_BACKEND=gcs`)

    `GCS_FILE_STORE_BUCKET_NAME`: Cloud Storage bucket name for file storage. Required.

    `GCS_FILE_STORE_PREFIX`: Object prefix for file storage. Defaults to `onyx-files`.

    `GCS_PROJECT_ID`: GCP project of the bucket. If you do not set it, Onyx gets the project from the credentials.

    `GCS_SERVICE_ACCOUNT_KEY_PATH`: Path to a service account JSON key file

    `GCS_SERVICE_ACCOUNT_KEY_JSON`: Service account JSON key as a string

    Onyx uses the first credentials it finds, in this order: `GCS_SERVICE_ACCOUNT_KEY_PATH`,
    then `GCS_SERVICE_ACCOUNT_KEY_JSON`, then Application Default Credentials.
    Application Default Credentials include GKE Workload Identity, the Compute Engine metadata server,
    and `gcloud auth application-default login`. On GKE, use Workload Identity and do not set a key.
    The identity needs `roles/storage.objectAdmin` and `roles/storage.legacyBucketReader` on the bucket,
    because Onyx reads the bucket when it starts.

    **Azure Blob Storage** (`FILE_STORE_BACKEND=azure`)

    `AZURE_FILE_STORE_CONTAINER_NAME`: Blob container name for file storage

    `AZURE_FILE_STORE_PREFIX`: Blob prefix for file storage. Defaults to `onyx-files`.

    `AZURE_STORAGE_ACCOUNT_NAME`: Storage account name

    `AZURE_STORAGE_ACCOUNT_URL`: Blob endpoint URL. If you do not set it,
    Onyx uses `https://<account>.blob.core.windows.net`. Set it for Azurite or sovereign clouds.

    `AZURE_STORAGE_CONNECTION_STRING`: Storage account connection string

    `AZURE_STORAGE_ACCOUNT_KEY`: Storage account key

    Onyx uses the first credentials it finds, in this order: `AZURE_STORAGE_CONNECTION_STRING`,
    then `AZURE_STORAGE_ACCOUNT_KEY`, then `DefaultAzureCredential`.
    `DefaultAzureCredential` includes AKS Workload Identity, managed identity, and `az login`.
  </Accordion>
