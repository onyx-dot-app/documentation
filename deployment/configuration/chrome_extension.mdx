...
<Tabs>
  ...
  <Tab title="Windows / macOS / Linux policy">
    Set the `3rdparty.extensions.<extension-id>` policy. Example for Linux,
    `/etc/opt/chrome/policies/managed/onyx.json`:
    ...
    On Windows, use the same keys under `Software\Policies\Google\Chrome\3rdparty\extensions\<extension-id>\policy`.
  </Tab>
</Tabs>
