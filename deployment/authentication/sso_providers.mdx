...
<Note>
  ...
  `AUTH_TYPE` no longer controls how Onyx authenticates and Onyx logs a warning while it is still set. Remove it along
  with the rest of the legacy variables once the provider row exists.
</Note>
...
- Accounts do not link across providers. A user account belongs to the sign-in
  method it first used. Signing in through a different provider with the same
  email fails with an error instead of merging the accounts. To move a user to
  a new provider, have an admin deactivate and then delete the account so the
  user can sign up fresh through it.
- If open sign-up is restricted (**Admin Panel** → **Users** → **Restrict Open
  Sign-Up**), SSO sign-ups are gated too. New users who have not been invited
  are rejected at sign-in until an admin invites their email address.
