...
<Note>
  ...
  so nothing changes on the IdP side.
  Keep the legacy configuration in place through the upgrade (the migration reads it),
  then remove it after the provider appears in the admin panel. From `v4.4.0` on,
  `AUTH_TYPE` no longer controls how Onyx authenticates and Onyx logs a warning while it is still set.
  Remove it along with the rest of the legacy variables once the provider row exists.
</Note>
...
- Accounts do not link across providers. A user account belongs to the sign-in
  method it first used.
  Signing in through a different provider with the same email fails with an error instead of merging the accounts.
  To move a user to a new provider,
...
- If open sign-up is restricted (**Admin Panel** → **Users** → **Restrict Open
  Sign-Up**), SSO sign-ups are gated too.
