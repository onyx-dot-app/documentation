...
Google providers are managed from the admin panel at **Admin Panel** → **Organization** → [SSO
...
plan, see [Plan Availability](/deployment/authentication/sso_providers#plan-availability)).
...
Versions before `v4.4.0` configure a single Google provider through environment variables.
This flow uses the redirect URI `https://YOUR_ONYX_DOMAIN.com/auth/oauth/callback` on the OAuth client.

<Note>
  An environment-based configuration is migrated to an SSO provider entry automatically when you upgrade to `v4.4.0` or
  later, and existing logins keep working. The migrated provider keeps using the redirect URI already registered in the
  Google Cloud console, so nothing changes on the Google side. Keep the configuration below in place through the upgrade,
