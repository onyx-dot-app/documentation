...
and you can configure more than one provider (more than one simultaneously enabled provider requires the Business plan,
see [Plan Availability](/deployment/authentication/sso_providers#plan-availability)).
SAML works on the standard Onyx images.
...
<Note>
  ...
  Keep the settings file mounted and `AUTH_TYPE=saml` set through the upgrade,
  then remove both after the provider appears in the admin panel.
</Note>

To configure the legacy flow,
copy `onyx/backend/onyx/configs/saml_config/template.settings.json` to `settings.json` in the same directory (the api
server reads `settings.json` from `SAML_CONF_DIR`, which defaults to that directory),
