...
<Steps>
  ...
  <Step title="Configure Okta Application">
    ...
    **Name** your application `Onyx` and upload the Onyx logo.
    ...
    Add a **Single sign-on URL** using the name you will give the provider in Onyx (a lowercase slug, e.g. `okta`):

      ```
      https://YOUR_ONYX_DOMAIN.com/api/auth/saml/YOUR_PROVIDER_NAME/callback
      ```
...
<Note>
  ...
  first starts on `v4.4.0` or later, and existing logins keep working.
