...
OIDC providers are managed from the admin panel at **Admin Panel** → **Organization** → [SSO
...
<Note>
  ...
  later, and existing logins keep working. The migrated provider keeps using the redirect URI already registered with
  your IdP, so nothing changes on the IdP side. Keep the configuration below in place through the upgrade,
