<Info>
  On Windows, the commands will be slightly different but all the steps are the same.
</Info>

<Steps>
  <Step title="Install Requirements">
    - [Docker](https://docs.docker.com/get-docker/)
    - [Docker Compose](https://docs.docker.com/compose/install/)
  </Step>

  <Step title="Clone the Onyx repo">
    ```bash
    git clone https://github.com/onyx-dot-app/onyx.git
    ```
  </Step>

  <Step title="Navigate to the compose files">
    ```bash
    cd onyx/deployment/docker_compose
    ```
  </Step>

  <Step title="Launch Onyx">
    <Note>
      Make sure Docker has enough memory! If using Docker Desktop, you must increase memory manually!
    </Note>

    To pull images from Docker Hub:

    ```bash
    docker compose -f docker-compose.dev.yml -p onyx-stack up -d --pull always --force-recreate
    ```

    To build images from source:

    ```bash
    docker compose -f docker-compose.dev.yml -p onyx-stack up -d --build --force-recreate
    ```

    <Tip>
      On default settings, you can access Onyx at localhost:3000 after your containers are up.
    </Tip>
  </Step>

  <Step title="(Optional) Configure your Deployment">
    <Warning>
      For ease of testing, the default deployment does not have User Authentication configured.
      See the [Configuration](/deployment/configuration/configuration) page for directions to enable it.
    </Warning>

    Configure your deployment using a `.env` file.

    ```bash
    cd ~/onyx-dot-app/onyx/deployment/docker_compose
    cp env.prod.template .env
    ```

    You'll need to restart Onyx after changing any `.env` variables.

    ```bash
    docker compose -f docker-compose.dev.yml -p onyx-stack down
    docker compose -f docker-compose.dev.yml -p onyx-stack up -d --pull always --force-recreate
    ```
  </Step>
</Steps>
